            ...
            <AnimatePresence>
                {
                    ...
                    (<motion.aside transition={{ duration: 0.3 }} initial={{ x: '100%' }} exit={{ x: '100%' }} animate={{ x: 0 }} className='fixed top-0 right-0 w-full xs:w-[350px] md:w-[400px] bg-white h-screen  py-4 z-[160]'>
                        <div className="">
                            <div className="cart-header mt-4 px-4 flex justify-end">
                                <button onClick={() => { globalcontext.setCartSideStatus(false); document.body.classList.remove('overflow-hidden') }} className='text-xl'>
                                    <AiOutlineClose />
                                </button>
                            </div>
                            <ul style={{ 'scrollbarWidth': 'thin' }} className="cart-list font-WorkSans mt-4 py-4 overflow-y-auto h-[90vh]">
                                {
