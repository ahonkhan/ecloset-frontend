...
export const TwoFactorStep1 = ({ setCurrentStep }) => {
    let email = 'aohinuzzaman420@gmail.com';
    return (
        <div className="step-2 py-4 w-full gap-2 flex items-center justify-center px-1 sm:px-4 ">

            <div className="w-full  flex justify-center flex-col">
                <div className="py-4">
                    <p className='text-sm font-WorkSans text-gray-800 text-center'>By clicking button you will get a security code to {email.slice(0, 3) + '...' + email.split('@')[0].slice(-2) + '...@' + email.split('@')[1]} </p>
                </div>
                <form onClick={(e) => e.preventDefault()} className="flex gap-2 items-center w-full flex-col sm:flex-row">
                    <div className="w-full flex justify-center">
                        <button onClick={() => setCurrentStep(1)} className='px-8 disabled:opacity-70 disabled:scale-100 py-3 w-full sm:w-fit duration-200 active:scale-95 rounded bg-darkBlue text-slate-50 font-WorkSans font-medium'>Send Otp</button>
                    </div>
                </form>
            </div>
        </div>
...
export const TwoFactorStep2 = ({ setCurrentStep }) => {
    const [isDisabled, setIsDisabled] = useState(true)
    const handleInput = (e) => {
        if (e.target.value) {
            setIsDisabled(false);
        ...
        <div className="step-2 py-4 w-full gap-2 flex items-center justify-center px-1 sm:px-4 ">
            ...
            <div className="w-full">
                <div className="py-4">
                    ...
                    <p className='text-sm font-WorkSans text-center text-gray-800'>You have received a security code in <span>aohinuzzaman420@gmail.com</span>. Submit it within 150 seconds</p>
                </div>
                <form onClick={(e) => e.preventDefault()} className="flex gap-2 items-center w-full flex-col sm:flex-row">
...
export const TwoFactorStep3 = ({ setCurrentStep, enabled, setEnabled }) => {
    const [isDisabled, setIsDisabled] = useState(true)
    const handleInput = (e) => {
        if (e.target.value) {
            setIsDisabled(false);
        ...
        <div className="step-3 w-full py-4 gap-2 flex items-center justify-center px-1 sm:px-4 ">
            ...
            <div className="w-full">
                <div className="py-4">
                    ...
                    <p className='text-sm font-WorkSans text-gray-800'>You can {enabled ? 'disable' : 'enable'} two factor authentication with your password, you don't have to do anything else.</p>
                </div>
                <form onClick={(e) => e.preventDefault()} className="flex gap-2 items-center w-full flex-col sm:flex-row">

                    <div className="input-item w-full relative bg-slate-200 flex items-center rounded ">
                        <input onChange={(e) => handleInput(e)} type={'text'} autoComplete='new-password' placeholder='Password' className='inputField outline-none focus:ring-1 pr-8' />
                    </div>
                    <div className="w-full sm:w-fit">
                        <button onClick={() => { setEnabled(!enabled); setCurrentStep(0) }} disabled={isDisabled} className='px-8 py-3 w-full sm:w-fit disabled:opacity-70 disabled:scale-100 duration-200 active:scale-95 rounded bg-darkBlue text-slate-50 font-WorkSans font-medium'>{enabled ? 'Disable' : 'Enable'}</button>
                    </div>
                </form>
            </div >
        </div >
